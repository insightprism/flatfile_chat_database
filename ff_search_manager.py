...
class FFSearchManager:
    """
    FF search manager for the flatfile chat database.
    ...
    async def build_search_index(self, user_id: str) -> Dict[str, Any]:
        ...
        user_path = ff_get_user_path(self.base_path, user_id, self.config)
    ...
    async def _search_messages(self, user_id: str, session_id: str,
        ...
        # Read messages
        messages_data = await ff_read_jsonl(messages_file, self.config)
    ...
    async def _search_documents(self, user_id: str, session_id: str,
        ...
        docs_metadata = await ff_read_json(metadata_file, self.config) or {}
    ...
    async def _index_session(self, user_id: str, session_id: str) -> Dict[str, Any]:
        ...
        if messages_file.exists():
            messages = await ff_read_jsonl(messages_file, self.config)
    ...
    async def vector_search(self, query: FFSearchQueryDTO) -> List[FFSearchResultDTO]:
        ...
        # Import FFStorageManager here to avoid circular imports
        from ff_storage_manager import FFStorageManager
        
        # Use FFStorageManager for vector search
        storage = FFStorageManager(self.config)
    ...
    async def search_enhanced(self, query: FFSearchQueryDTO) -> List[FFSearchResultDTO]:
        ...
        if query.hybrid_search:
            # Use hybrid search from FFStorageManager
            from ff_storage_manager import FFStorageManager
            storage = FFStorageManager(self.config)
