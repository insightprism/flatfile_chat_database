...
class FFDatabaseAdapterManager(ABC):
...
class FFSQLiteAdapter(FFDatabaseAdapterManager):
...
class FFFlatfileExporter:
    ...
    def __init__(self, storage_manager: FFStorageManager,
        ...
        self.streamer = FFExportStreamerManager(self.config)
        self.compression = FFCompressionManager(self.config, compression_config)
    ...
    async def import_from_json(self, json_path: Path,
        ...
        try:
            ...
            if json_path.suffix == '.gz':
                compression = FFCompressionManager(self.config)
