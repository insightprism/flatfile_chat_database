...
class FFCompressionManager:
    ...
    def __init__(self, config: FFConfigurationManagerConfigDTO, 
        ...
        """
        self.config = config
        self.compression_config = compression_config or FFCompressionConfig()
    ...
    async def compress_data(self, data: Union[str, bytes],
        ...
        elif self.compression_config.type == FFCompressionType.ZLIB:
    ...
    async def _detect_compression(self, data: bytes) -> FFCompressionType:
        """Detect compression type from data"""
        if len(data) < 2:
            return FFCompressionType.NONE
        
        # Check magic numbers
        if data[:2] == b'\x1f\x8b':  # gzip magic number
            return FFCompressionType.GZIP
        elif data[:2] == b'\x78\x9c' or data[:2] == b'\x78\x01':  # zlib magic numbers
            return FFCompressionType.ZLIB
        
        return FFCompressionType.NONE
    
    async def _detect_file_compression(self, file_path: Path) -> FFCompressionType:
