...
from utils.file_ops import atomic_write, atomic_append, safe_read
...
async def write_json(
    ...
    try:
        ...
        # Write atomically
        return await atomic_write(path, json_str, config, mode='w')
        
    except (TypeError, ValueError) as e:
        raise JSONError(f"Failed to serialize data: {str(e)}")
...
async def read_jsonl_paginated(
    ...
    entries = await read_jsonl(path, config, limit=page_size, offset=offset)
    
    # Count total entries for pagination metadata
    total = await count_jsonl_entries(path)
...
async def count_jsonl_entries(path: Path) -> int:
    """
...
async def update_jsonl_entry(
    ...
    # Read all entries
    entries = await read_jsonl(path, config)
...
async def merge_json_files(
    ...
    for file_path in files:
        data = await read_json(file_path, config)
        if data:
            merged.update(data)
    
    return await write_json(output_path, merged, config)