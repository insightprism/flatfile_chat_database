...
async def test_storage_manager_integration():
    ...
    try:
        # Initialize storage manager with test data path
        config = StorageConfig()
        config.storage_base_path = "./test_prismmind_data"
        ...
        if not storage.is_prismmind_available():
            ...
            print("   This might be due to missing PrismMind dependencies")
            return False
...
async def test_legacy_fallback():
    ...
    try:
        ...
        config = StorageConfig()
        config.storage_base_path = "./test_legacy_data"
...
async def test_configuration_loading():
    ...
    try:
        ...
        config_path = str(main_db_path / "configs" / "flatfile_prismmind_config.json")
...
async def test_prismmind_availability():
    """Test if PrismMind modules are available"""
    print("\n=== Testing PrismMind Module Availability ===")
    
    try:
        # Try importing PrismMind modules
        test_imports = [
            'pm_trace_logger',
            'pm_engine_config',
            'pm_run_engine_chain',
        ...
        if missing_modules:
            print(f"\n⚠️ Missing PrismMind modules: {missing_modules}")
            print("   PrismMind integration will not work without these modules")
            return False
        else:
            print(f"\n✓ All PrismMind modules available: {available_modules}")
            return True
            
    except Exception as e:
        print(f"❌ PrismMind availability test failed: {e}")
        return False
...
async def main():
    ...
    # Test PrismMind module availability first
    test_results["prismmind_modules"] = await test_prismmind_availability()
    
    # Test configuration loading
    test_results["config"] = await test_configuration_loading()
    
    # Only run integration tests if PrismMind modules are available
    if test_results["prismmind_modules"]:
        ...
        print("\n⚠️ Skipping integration tests due to missing PrismMind modules")
        test_results["storage"] = False
        test_results["pipeline"] = False
    
    # Test legacy fallback (should always work)
    ...
    if passed == total:
        ...
        return True
    elif test_results["legacy"]:
        print("⚠️  PrismMind integration not available, but legacy processing works.")
        print("   To enable PrismMind integration, ensure PrismMind modules are installed.")
        return True
    else:
        print("❌ Critical failures detected. Check the output above for details.")
        return False
