...
class FFVectorStorageManager:
    ...
    async def store_vectors(
        ...
        if embeddings_path.exists():
            ...
            existing_index = await ff_read_jsonl(index_path, self.config)
        else:
        ...
        for entry in new_index_entries:
            await ff_append_jsonl(index_path, entry, self.config)
    ...
    async def load_vectors(
        ...
        index = await ff_read_jsonl(index_path, self.config)
    ...
    async def delete_document_vectors(
        ...
        if len(keep_indices) > 0:
            ...
            # Rewrite index
            await ff_atomic_write(index_path, "", self.config)  # Clear file
