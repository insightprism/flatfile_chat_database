...
class FFPrismMindConfigLoader:
    ...
    def from_environment(prefix: str = "FLATFILE_PM_") -> FFPrismMindConfig:
        ...
        config_data = FFPrismMindConfigLoader._get_default_config_dict()
        ...
        for key, value in os.environ.items():
            if key.startswith(prefix):
                ...
                FFPrismMindConfigLoader._set_nested_value(env_overrides, config_path, value)
        
        # Merge environment overrides
        config_data = FFPrismMindConfigLoader._deep_merge(config_data, env_overrides)
    ...
    def from_dict(config_dict: Dict[str, Any]) -> FFPrismMindConfig:
        ...
        try:
            ...
            doc_processing_config = FFDocumentProcessingConfig(**doc_processing_data)
            
            # Extract engine selection config
            engine_selection_data = config_dict.get("engine_selection", {})
            engine_selection_config = FFEngineSelectionConfig(**engine_selection_data)
            
            # Extract handler strategies config
            handler_strategies_data = config_dict.get("handler_strategies", {})
            handler_strategies_config = FFHandlerStrategiesConfig(**handler_strategies_data)
            
            # Extract integration config
            integration_data = config_dict.get("integration_settings", {})
            integration_config = FFIntegrationConfig(**integration_data)
    ...
    def merge_configs(
        ...
        merged_dict = FFPrismMindConfigLoader._deep_merge(base_dict, override_config)
        
        return FFPrismMindConfigLoader.from_dict(merged_dict)
    ...
    def load_with_fallbacks(
        ...
        if primary_path and Path(primary_path).exists():
            try:
                config = FFPrismMindConfigLoader.from_file(primary_path)
        ...
        if not config and fallback_paths:
            for fallback_path in fallback_paths:
                if Path(fallback_path).exists():
                    try:
                        config = FFPrismMindConfigLoader.from_file(fallback_path)
        ...
        if not config:
            config = FFPrismMindConfigLoader.create_default_config()
            print("Using default configuration")
        
        # Apply environment variable overrides
        try:
            env_overrides = FFPrismMindConfigLoader._get_environment_overrides(environment_prefix)
            if env_overrides:
                config = FFPrismMindConfigLoader.merge_configs(config, env_overrides)
    ...
    def validate_config_file(config_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                config = FFPrismMindConfigLoader.from_file(config_path)
                validation_result["structure_valid"] = True
                
                # Run configuration validation
                from .ff_prismmind_engine_factory import FFPrismMindConfigFactory
                config_errors = FFPrismMindConfigFactory.validate_configuration(config)
    ...
    def _get_default_config_dict() -> Dict[str, Any]:
        ...
        default_config = FFPrismMindConfigLoader.create_default_config()
    ...
    def _get_environment_overrides(prefix: str) -> Dict[str, Any]:
        ...
        for key, value in os.environ.items():
            if key.startswith(prefix):
                ...
                converted_value = FFPrismMindConfigLoader._convert_env_value(value)
                
                # Set nested value
                FFPrismMindConfigLoader._set_nested_value(overrides, config_path, converted_value)
    ...
    def _deep_merge(base: Dict[str, Any], override: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in override.items():
            if key in result and isinstance(result[key], dict) and isinstance(value, dict):
                result[key] = FFPrismMindConfigLoader._deep_merge(result[key], value)
            else:
                result[key] = value
