...
This processor replaces the monolithic DocumentRAGPipeline with a clean,
...
from flatfile_chat_database.models import ProcessingResult


class FlatfileDocumentProcessor:
    """
    Main integration class - uses PrismMind engines with flatfile storage.
    
    Replaces DocumentRAGPipeline with configuration-driven PrismMind approach.
    """
    
    def __init__(self, config: FlatfilePrismMindConfig):
        """
        ...
        """
        self.config = config
        self.storage_manager = StorageManager(config.flatfile_config)
        self.factory = FlatfilePrismMindConfigFactory()
    ...
    async def process_batch(
        ...
        async def process_single_document(doc_path: str) -> ProcessingResult:
    ...
    async def reprocess_document(
        ...
        Returns:
            ProcessingResult with reprocessing status
        """
        try:
