...
class FFSearchQueryProtocol(Protocol):
    """
...
class FFSearchResultProtocol(Protocol):
    """
    ...
    def __lt__(self, other: 'FFSearchResultProtocol') -> bool:
        """Compare results by relevance score."""
        ...


class FFSearchProtocol(Protocol):
    """
    ...
    async def search(self, query: FFSearchQueryProtocol) -> List[FFSearchResultProtocol]:
        """
    ...
    async def search_by_text(self, text: str, user_id: Optional[str] = None,
                           limit: int = 100) -> List[FFSearchResultProtocol]:
        """
        Simple text search.
    ...
    async def search_by_vector(self, embedding: List[float], 
                             ...
                             limit: int = 100,
                             threshold: float = 0.7) -> List[FFSearchResultProtocol]:
        """
    ...
    async def hybrid_search(self, text: str, embedding: List[float],
                          user_id: Optional[str] = None,
                          limit: int = 100,
                          text_weight: float = 0.5) -> List[FFSearchResultProtocol]:
        """
