...
class FFAtomicWriteError(FFFileOperationError):
    """Error during atomic write operation"""
    pass


class FFLockTimeoutError(FFFileOperationError):
        ...
        while True:
            try:
                ...
                if time.time() - start_time > self.timeout:
                    raise FFLockTimeoutError(f"Could not acquire lock on {self.path} within {self.timeout}s")
...
class FileOperationManager:
    ...
    def __init__(self, config: FFConfigurationManagerConfigDTO):
        ...
        """
        self.config = config
        self._locks: Dict[Path, FFFileLock] = {}
        self._lock_pool = asyncio.Lock()
        
    async def _get_lock(self, path: Path) -> FFFileLock:
        """
        ...
        async with self._lock_pool:
            if canonical_path not in self._locks:
                ...
                self._locks[canonical_path] = FFFileLock(canonical_path, timeout=lock_timeout, config=self.config)
    ...
    async def _write_internal(self, path: Path, data: Union[str, bytes], write_mode: str = 'wb') -> bool:
        ...
        try:
            ...
            try:
                ...
                pass
            raise FFAtomicWriteError(f"Failed to write {path}: {str(e)}")
...
async def ff_atomic_write(
    ...
    Raises:
        FFAtomicWriteError: If write operation fails
    """
    manager = await ff_get_file_operation_manager(config)
    return await manager.execute('write', path, data, mode='exclusive', write_mode=mode)


async def ff_safe_read(
    path: Path,
    mode: str = 'rb',
    encoding: str = 'utf-8',
...
async def ff_ensure_directory(path: Path) -> bool:
    """
...
async def ff_safe_delete(
    ...
    try:
        ...
        if config.auto_cleanup_empty_directories:
            await ff_cleanup_empty_directories(path.parent)
...
async def ff_cleanup_empty_directories(path: Path) -> None:
    """
...
async def ff_copy_file(
    ...
    try:
        # Read source
        data = await ff_safe_read(source, mode='rb')
        if data is None:
            return False
...
async def ff_move_file(
    ...
    try:
        ...
        if await ff_copy_file(source, destination, config):
            return await ff_safe_delete(source, config)
        return False


async def ff_get_file_size(path: Path) -> int:
    """
...
async def ff_file_exists(path: Path) -> bool:
    """
...
async def ff_directory_exists(path: Path) -> bool:
    """
...
async def ff_get_directory_size(directory: Path) -> int:
    """
